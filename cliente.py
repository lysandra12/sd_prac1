...
def leer_archivo_linea_por_linea(nombre_archivo):
    ...
    with open(nombre_archivo, 'r') as archivo:
        for linea in archivo:
            ...
            partes = linea.split()
            yield Clientes(partes[1], partes[2], partes[0])
...
def main():
    ...
    generador = leer_archivo_linea_por_linea("benchmark_unnumbered_20000.txt")  # Crear generador UNA vez
    
    inicio = time.time()
    for cliente in generador:  # Iterar sobre el generador
        try:
            exito, resultado = lb.generar_ticket()
            print(f"Cliente: {cliente}, Compra con exito: {resultado}")
